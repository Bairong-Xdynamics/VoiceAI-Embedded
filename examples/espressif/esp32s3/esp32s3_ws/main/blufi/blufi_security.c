...
{
    if (data == NULL || len < 3) {
        ESP_LOGE(BLU_WIFI_TAG, "BLUFI Invalid data format");
    ...
    if (blufi_sec == NULL) {
        ESP_LOGE(BLU_WIFI_TAG, "BLUFI Security is not initialized");
    ...
    case SEC_TYPE_DH_PARAM_LEN:
        ...
        if (blufi_sec->dh_param == NULL) {
            ...
            ESP_LOGE(BLU_WIFI_TAG, "%s, malloc failed", __func__);
            return;
        }
        break;
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            ESP_LOGE(BLU_WIFI_TAG, "%s, blufi_sec->dh_param == NULL", __func__);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }

        if (len < (blufi_sec->dh_param_len + 1)) {
            ESP_LOGE(BLU_WIFI_TAG, "%s, invalid dh param len", __func__);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (ret) {
            ESP_LOGE(BLU_WIFI_TAG, "%s read param failed %d", __func__, ret);
        ...
        if (dhm_len > DH_SELF_PUB_KEY_LEN) {
            ESP_LOGE(BLU_WIFI_TAG, "%s dhm len not support %d", __func__, dhm_len);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (ret) {
            ESP_LOGE(BLU_WIFI_TAG, "%s make public failed %d", __func__, ret);
        ...
        ret = mbedtls_dhm_calc_secret( &blufi_sec->dhm,
                ...
                myrand, NULL);
        if (ret) {
            ESP_LOGE(BLU_WIFI_TAG, "%s mbedtls_dhm_calc_secret failed %d", __func__, ret);
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (ret) {
            ESP_LOGE(BLU_WIFI_TAG, "%s mbedtls_md5 failed %d", __func__, ret);
