...
/*
    ...
    protocol_ = std::make_unique<RtcProtocol>(CONFIG_CONNECTION_CONFIG_KEY, CONFIG_CONNECTION_CONFIG_TOKEN);
...
*/
    ...
    callbacks.on_encode_queue_overflow = [this]() {
        xEventGroupSetBits(event_group_, MAIN_EVENT_ENCODE_QUEUE_OVERFLOW);
    };
    audio_service_.SetCallbacks(callbacks);
...
void Application::MainEventLoop() {
    while (true) {
        ...
        if (bits & MAIN_EVENT_ENCODE_QUEUE_OVERFLOW) {
            ESP_LOGW(TAG, "Encode queue overloaded, closing audio channel");
