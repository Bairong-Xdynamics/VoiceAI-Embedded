...
private:
    ...
    std::mutex playback_ref_mutex_;
    std::deque<int16_t> playback_ref_ring_;
    static constexpr size_t PLAYBACK_REF_RING_SAMPLES = 16000 * 0.2; 
#endif
...
#if CONFIG_USE_DEVICE_AEC
    void PushPlaybackRef(const int16_t* pcm, size_t samples);
    void ConsumePlaybackRef(size_t need, std::vector<int16_t>& out);
#endif
    void PcmTask();
