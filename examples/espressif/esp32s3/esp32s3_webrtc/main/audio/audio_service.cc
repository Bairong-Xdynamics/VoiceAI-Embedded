...
AudioService::~AudioService() {
    ...
    if (opus_codec_task_tcb_ != nullptr) {
        heap_caps_free(opus_codec_task_tcb_);
    }
#endif
}
...
void AudioService::Start() {
    ...
    //xTaskCreate([](void* arg) {
    //    AudioService* audio_service = (AudioService*)arg;
    //    audio_service->PcmTask();
    //    vTaskDelete(NULL);
    //}, "pcm_task", 2048 * 4, this, 2, nullptr);

    /* Start the opus codec task */
#if CONFIG_SPIRAM
    if (opus_codec_task_stack_ == nullptr) {
        opus_codec_task_stack_ = (StackType_t*)heap_caps_malloc(kOpusCodecTaskStackSize, MALLOC_CAP_SPIRAM);
        assert(opus_codec_task_stack_ != nullptr);
    }
    if (opus_codec_task_tcb_ == nullptr) {
        opus_codec_task_tcb_ = (StaticTask_t*)heap_caps_malloc(sizeof(StaticTask_t), MALLOC_CAP_INTERNAL);
        assert(opus_codec_task_tcb_ != nullptr);
    }
    opus_codec_task_handle_ = xTaskCreateStatic([](void* arg) {
        AudioService* audio_service = (AudioService*)arg;
        audio_service->OpusCodecTask();
        vTaskDelete(NULL);
    }, "opus_codec", kOpusCodecTaskStackSize, this, 2, opus_codec_task_stack_, opus_codec_task_tcb_);
#else
    xTaskCreate([](void* arg) {
        AudioService* audio_service = (AudioService*)arg;
        audio_service->OpusCodecTask();
        vTaskDelete(NULL);
    }, "opus_codec", 2048 * 13, this, 4, &opus_codec_task_handle_);
#endif
}
...
#if CONFIG_USE_DEVICE_AEC
    {
        std::lock_guard<std::mutex> ref_lock(playback_ref_mutex_);
        playback_ref_ring_.clear();
    }
#endif
}
...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        if (bits & AS_EVENT_AUDIO_PROCESSOR_RUNNING) {
            ...
            if (samples > 0) {
                if (ReadAudioData(data, 16000, samples)) {
                    const bool mic_is_mono =
#if CONFIG_USE_DEVICE_AEC
                        !codec_->input_reference();
#else
                        codec_->input_channels() == 1;
#endif
                    UpdatePcmVoiceFromCapturedInput(data, static_cast<size_t>(samples), mic_is_mono);
#if CONFIG_USE_DEVICE_AEC
                    if (!codec_->input_reference()) {
                        std::vector<int16_t> ref;
                        ConsumePlaybackRef(static_cast<size_t>(samples), ref);
                        std::vector<int16_t> interleaved(static_cast<size_t>(samples) * 2);
                        for (int i = 0; i < samples; i++) {
                            interleaved[static_cast<size_t>(i) * 2] = data[static_cast<size_t>(i)];
                            interleaved[static_cast<size_t>(i) * 2 + 1] = ref[static_cast<size_t>(i)];
                        }
...
#if CONFIG_USE_DEVICE_AEC
        if (!codec_->input_reference()) {
            PushPlaybackRef(task->pcm.data(), task->pcm.size());
        }
#endif
...
void AudioService::PcmTask() {
    while (true) {
        std::unique_lock<std::mutex> lock(audio_queue_mutex_);
        audio_queue_cv_.wait(lock, [this]() { return !audio_pcm_queue_.empty() || service_stopped_; });
        if (service_stopped_) {
            break;
        }

        auto packet = std::move(audio_pcm_queue_.front());
        audio_pcm_queue_.pop_front();
        audio_queue_cv_.notify_all();
        lock.unlock();

        size_t bytes = packet->payload.size();
        if (bytes % 2 != 0) bytes -= 1;
        size_t samples = bytes / 2;

        auto task = std::make_unique<AudioTask>();
        task->type = kAudioTaskTypeDecodeToPlaybackQueue;
        task->timestamp = packet->timestamp;
        task->pcm.resize(samples);
        std::memcpy(task->pcm.data(), packet->payload.data(), bytes);
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        /* Encode the audio to send queue (prioritize uplink, decode already has a large buffer) */
...
#if CONFIG_USE_WS_DEBUG_SINK
                if (WsLogPcmSink::Instance().IsEnabled()) {
                    auto* pcm = reinterpret_cast<const int16_t*>(task->pcm.data());
                    size_t samples = task->pcm.size() / sizeof(int16_t);
                    WsLogPcmSink::Instance().EnqueuePcm(16000, 1, pcm, samples);
                }
#endif
                // Resample if the sample rate is different
...
#if CONFIG_USE_DEVICE_AEC
void AudioService::PushPlaybackRef(const int16_t* pcm, size_t samples) {
    if (pcm == nullptr || samples == 0) {
        return;
    }
    std::lock_guard<std::mutex> lock(playback_ref_mutex_);
    if (codec_->output_sample_rate() == 16000) {
        for (size_t i = 0; i < samples; i++) {
            playback_ref_ring_.push_back(pcm[i]);
            if (playback_ref_ring_.size() > PLAYBACK_REF_RING_SAMPLES) {
                playback_ref_ring_.pop_front();
            }
        }
    } else {
        size_t out_size = playback_ref_resampler_.GetOutputSamples(samples);
        std::vector<int16_t> resampled(out_size);
        playback_ref_resampler_.Process(pcm, samples, resampled.data());
        for (int16_t s : resampled) {
            playback_ref_ring_.push_back(s);
            if (playback_ref_ring_.size() > PLAYBACK_REF_RING_SAMPLES) {
                playback_ref_ring_.pop_front();
            }
        }
...
void AudioService::ConsumePlaybackRef(size_t need, std::vector<int16_t>& out) {
    out.resize(need, 0);
    std::lock_guard<std::mutex> lock(playback_ref_mutex_);
    size_t take = std::min(need, playback_ref_ring_.size());
    for (size_t i = 0; i < take; i++) {
        out[i] = playback_ref_ring_.front();
        playback_ref_ring_.pop_front();
    }
}
#endif

void AudioService::PushTaskToPcmQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    auto packet = std::make_unique<AudioStreamPacket>();
    packet->frame_duration = OPUS_FRAME_DURATION_MS;
    packet->sample_rate = 16000;
    packet->timestamp = 0;
    packet->payload.resize(pcm.size() * 2);
    std::memcpy(packet->payload.data(), pcm.data(), pcm.size() * sizeof(int16_t));
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    audio_send_queue_.push_back(std::move(packet));
    audio_queue_cv_.notify_all();
    if (callbacks_.on_send_queue_available) {
...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    if (type == kAudioTaskTypeEncodeToSendQueue) {
        if (audio_encode_queue_.size() >= MAX_ENCODE_TASKS_IN_QUEUE) {
            ...
            if (callbacks_.on_encode_queue_overflow) {
                callbacks_.on_encode_queue_overflow();
            }
            return;
        }
...
bool AudioService::PushPacketToPcmQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait) {
#if CONFIG_USE_WS_DEBUG_SINK
    if (WsLogPcmSink::Instance().IsEnabled()) {
        auto* pcm = reinterpret_cast<const int16_t*>(packet->payload.data());
        size_t samples = packet->payload.size() / sizeof(int16_t);
        WsLogPcmSink::Instance().EnqueuePcm(16000, 1, pcm, samples);
    }
#endif
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    audio_pcm_queue_.push_back(std::move(packet));
    audio_queue_cv_.notify_all();
    return true;
}
